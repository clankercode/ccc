...
- [ ] Add field to config: `prompt_mode`: default|append|prepend
  - Allows aliases like `ccc @add-task '<a new feature>'`.
  - Default: send prompt as-is to underlying CLI.
  - Append: add config-specified suffix text after user prompt.
  - Prepend: add config-specified prefix text before user prompt.
